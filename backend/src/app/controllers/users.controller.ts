...
export function createUsersController(users: UsersService): UsersController {
  return {
    list: async (_request, response, next) => {
      try { response.json(await users.list()); } catch (error) { next(error); }
    },
    create: async (request, response, next) => {
      try { response.status(201).json(await users.create(parse(createUserSchema, request.body))); } catch (error) { next(error); }
    },
    get: async (request, response, next) => {
      try { response.json(await users.get(parse(userIdSchema, request.params).id)); } catch (error) { next(error); }
    },
